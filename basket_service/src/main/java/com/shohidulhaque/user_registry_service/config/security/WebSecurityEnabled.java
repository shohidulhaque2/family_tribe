...
//  }
    ...
    Optional<Converter<Jwt, ? extends AbstractAuthenticationToken>> getJwtAuthenticationConverter() {
        ...
        if (this.applicationContext.getBeanNamesForType(JwtAuthenticationConverter.class).length
            > 0) {
            //what if their are multiple types found?
            jwtAuthenticationConverter = this.applicationContext
...
//        .oauth2ResourceServer(OAuth2ResourceServerConfigurer::jwt);
            ...
            new JwtAccessTokenToJwtAuthenticationTokenTransformer(getJwtAuthenticationConverter()));
